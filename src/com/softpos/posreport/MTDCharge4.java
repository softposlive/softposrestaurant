...
public class MTDCharge4 extends javax.swing.JDialog {
    ...
    public MTDCharge4(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("รายงานสรุปการเบิกสินค้ารวมตามหน่วยงาน/บุคคล");
        ...
        try {
            ...
            if (!Value.getComPort().equals("NONE")) {
                if (prn.OpenPrint(Value.getComPort())) {
                    ...
                    prn.print(" รายงานสรุปการเบิกสินค้ารวมตามหน่วยงาน/บุคคล");
                    ...
                    prn.print("ชื่อชาร์จหน่วยงาน/พนักงาน           รหัสชาร์จ ");
                    prn.print("วันที่ (Date)        เวลา         จำนวนเงิน ");
                    prn.print("----------------------------------------");
                    try {
                        Statement stmt =  MySQLConnect.con.createStatement();
                        String SqlQuery = "select b_chargegroup,b_chargecode,b_chargename,b_ondate,b_ontime,b_total from s_charge " +
                                "where (s_date>='" + Datefmt.format(TDate1) + "') and (s_date<='" + Datefmt.format(TDate2) + "') and (b_chargecode>='" + Charge1 + "') and (b_chargecode<='" + Charge2 + "') " +
                                " and (b_void<>'V') order by b_chargegroup,b_chargecode,b_ondate,b_ontime";
                        ...
                        if (rec.getRow() == 0) {
                            ...
                            do {
                                ...
                                if (!TempCharge.equals(rec.getString("b_chargecode"))) {
                                    prn.print(PUtility.DataFullR(rec.getString("b_chargename"),25)+"  "+rec.getString("b_chargecode"));
                                    TempCharge = rec.getString("b_chargecode") ;
                                }
                                prn.print(DatefmtShow.format(rec.getDate("b_ondate"))+"      "+rec.getString("b_ontime")+" "+ PUtility.DataFull(DecFmt.format(rec.getDouble("b_total")), 13));
                                if (rec.getString("b_chargegroup").equals("1")) {
                                    SumCnt1 = SumCnt1+1 ;
                                    SumAmt1 = SumAmt1+rec.getDouble("b_total") ;
                                } else {
                                    SumCnt2 = SumCnt2+1 ;
                                    SumAmt2 = SumAmt2+rec.getDouble("b_total") ;
                                }
                            } while (rec.next());
                        }
                        rec.close();
